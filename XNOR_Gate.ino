...
void loop() {
  ...
  //the '||' is the function that checks that EITHER inputs is 1
  //next step is to check if BOTH are 1; if so then turn the gate ON
  if(buttonA || buttonB){
    if(buttonA && buttonB){
      digitalWrite(GATE_LED_PIN, HIGH);
    }
    else{
      digitalWrite(GATE_LED_PIN, LOW); //only one input is 1
    }
  }
  else{
    digitalWrite(GATE_LED_PIN, HIGH); //turn it ON if NEITHER input is 1
  }
